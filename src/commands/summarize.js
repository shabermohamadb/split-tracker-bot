...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    .setName('summarize')
    .setDescription('Summarize a creative writing draft or lore document')
    .addStringOption(option =>
      option.setName('draft')
        .setDescription('Paste your writing draft or text here')
        .setRequired(false)
    )
    .addAttachmentOption(option =>
      option.setName('file')
        .setDescription('Or upload a text/markdown document (.txt, .md)')
        .setRequired(false)
    )
  ...
  async execute(interaction) {
    ...
    await aiModule.executeCommand(interaction, 'summarize', {
      draft,
      file,
      ephemeral,
      model
